...
@external("env", "begin_path")
    declare function begin_path(): void;

@external("env", "stroke")
    declare function stroke(): void;

@external("env", "move_to")
    declare function move_to(x: f64, y: f64): void;

@external("env", "line_to")
    declare function line_to(x: f64, y: f64): void;


// Called whenever a user clicks. Each user has a unique ID.
// Spawn a ball when the player clicks.
export function add_line_segment(x0: f64, y0: f64, x1: f64, y1: f64): void {
    line_segments.push(new Point(x0, y0));
    line_segments.push(new Point(x1, y1));
}
...
export function fixed_update(): void {
    ...
    for (let i = 0; i < balls.length; ++i) {
        let ball = balls[i];
        let below_y = (ball.position.y + ball.radius) - 400;
        if (below_y > 0) {
...
export function draw(): void {
    ...
    let previous = line_segments[0];

    begin_path();
    for (let i = 0; i < line_segments.length; i += 2) {
        let a = line_segments[i];
        let b = line_segments[i + 1];
        //  move_to(a.x, a.y);
        line_to(b.x, b.y);
    }
    stroke();
...
let line_segments = new Array<Point>();
